...
for (seed in 2^seq(1,10)){
	...
	kknnGrid <- expand.grid(kmax  = 30, distance=c(0.03, 0.05, 0.07, 0.1, 0.3), kernel=c("rectangular", "triangular", "epanechnikov", "biweight", "triweight", "cos", "inv", "gaussian"))
	set.seed(seed)
	...
	plot_scatter(pred=svmFit_pred, obs=MDFP_test$pIC50, FP="ALLFP", ML=paste0("SVM", "_seed", seed))

	svm_grid_plot <- ggplot(svmFit$results, aes(log(sigma), log(C), fill=RMSE)) + geom_raster() + scale_fill_continuous(low="green", high="red")
	ggsave(svm_grid_plot, file = "tuneplots/svm_grid_plot_ALLFP.pdf")
	...
	plot_scatter(pred=gbmFit_pred, obs=MDFP_test$pIC50, FP="ALLFP", ML=paste0("GBM", "_seed", seed))

	gbm_grid_plot <- ggplot(gbmFit$results, aes(n.trees, RMSE, color = as.factor(interaction.depth))) + geom_point() + geom_line()
	ggsave(gbm_grid_plot, file = "tuneplots/gbm_grid_plot_ALLFP.pdf")
	...
	plot_scatter(pred=rfFit_pred, obs=MDFP_test$pIC50, FP="ALLFP", paste0("RF", "_seed", seed))

	rf_grid_plot <- ggplot(rfFit$results, aes(mtry, RMSE)) + geom_point() + geom_line()
	ggsave(rf_grid_plot, file = "tuneplots/rf_grid_plot_ALLFP.pdf")
	...
	plot_scatter(pred=kknnFit_pred, obs=MDFP_test$pIC50, FP="ALLFP", ML=paste0("kknn", "_seed", seed))

	kknn_grid_plot <- ggplot(kknnFit$results, aes(distance, RMSE, color = kernel)) + geom_point() + geom_line() 
	ggsave(kknn_grid_plot, file = "tuneplots/kknn_grid_plot_ALLFP.pdf")
	...
	plot_scatter(pred=consensus_pred, obs=MDFP_test$pIC50, FP="ALLFP", ML=paste0("consensus", "_seed", seed))
	...
	plot_scatter(pred=consensus_pred_2, obs=MDFP_test$pIC50, FP="ALLFP", ML=paste0("consensus_2", "_seed", seed))
...
write.csv(metric_cv_all, file="ALLFP_metric_cv_all.csv")
write.csv(metric_test_all, file="ALLFP_metric_test_all.csv")

write.csv(metric_cv_all_summary, file="ALLFP_metric_cv_all_summary.csv")
write.csv(metric_test_all_summary, file="ALLFP_metric_test_all_summary.csv")

save.image("ALLFP-ML.rData")
