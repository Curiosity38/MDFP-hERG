...
MDFP <- read.csv("BaselineFP.csv", stringsAsFactors  = FALSE, header = TRUE)
...
if (do_rfe_select){
	...
	MDFP_rfe_predictors <- read.csv(file = "BaselineFP_rfe_predictors.csv", row.names = 1)
...
for (seed in 2^seq(1,10)){
	...
	ggsave(svm_grid_plot, file = "tuneplots/svm_grid_plot_BaselineFP.pdf")
	...
	ggsave(gbm_grid_plot, file = "tuneplots/gbm_grid_plot_BaselineFP.pdf")
	...
	ggsave(rf_grid_plot, file = "tuneplots/rf_grid_plot_BaselineFP.pdf")
	...
	plot_scatter(pred=kknnFit_pred, obs=MDFP_test$pIC50, FP="BaselineFP", ML=paste0("kknn", "_seed", seed))

	kknn_grid_plot <- ggplot(kknnFit$results, aes(distance, RMSE, color = kernel)) + geom_point() + geom_line() 
	ggsave(kknn_grid_plot, file = "tuneplots/kknn_grid_plot_BaselineFP.pdf")
	...
	plot_scatter(pred=consensus_pred_2, obs=MDFP_test$pIC50, FP="BaselineFP", ML=paste0("consensus_2", "_seed", seed))
