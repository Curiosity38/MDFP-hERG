...
MDFP <- read.csv("3D11-7837.csv", stringsAsFactors  = FALSE, header = TRUE)
...
if (do_rfe_select){
	...
	write.csv(predictors(rfeProfile), file = "3DFP11_rfe_predictors.csv")

	#选择RFE选出的特征
	MDFP_train <- select(MDFP_train, pIC50, predictors(rfeProfile))
}else{
	MDFP_rfe_predictors <- read.csv(file = "3DFP11_rfe_predictors.csv", row.names = 1)
...
for (seed in 2^seq(1,10)){
	...
	svmGrid <- expand.grid(sigma = exp(1)^seq(2,7,1), 
						   C = exp(1)^seq(-1,6,1))
	set.seed(seed)
	...
	plot_scatter(pred=svmFit_pred, obs=MDFP_test$pIC50, FP="3DFP11", ML=paste0("SVM", "_seed", seed))

	svm_grid_plot <- ggplot(svmFit$results, aes(log(sigma), log(C), fill=RMSE)) + geom_raster() + scale_fill_continuous(low="green", high="red")
	ggsave(svm_grid_plot, file = "tuneplots/svm_grid_plot_3DFP11.pdf")
	...
	plot_scatter(pred=gbmFit_pred, obs=MDFP_test$pIC50, FP="3DFP11", ML=paste0("GBM", "_seed", seed))

	gbm_grid_plot <- ggplot(gbmFit$results, aes(n.trees, RMSE, color = as.factor(interaction.depth))) + geom_point() + geom_line()
	ggsave(gbm_grid_plot, file = "tuneplots/gbm_grid_plot_3DFP11.pdf")
	...
	plot_scatter(pred=rfFit_pred, obs=MDFP_test$pIC50, FP="3DFP11", paste0("RF", "_seed", seed))

	rf_grid_plot <- ggplot(rfFit$results, aes(mtry, RMSE)) + geom_point() + geom_line()
	ggsave(rf_grid_plot, file = "tuneplots/rf_grid_plot_3DFP11.pdf")
	...
	plot_scatter(pred=kknnFit_pred, obs=MDFP_test$pIC50, FP="3DFP11", ML=paste0("kknn", "_seed", seed))

	kknn_grid_plot <- ggplot(kknnFit$results, aes(distance, RMSE, color = kernel)) + geom_point() + geom_line() 
	ggsave(kknn_grid_plot, file = "tuneplots/kknn_grid_plot_3DFP11.pdf")
	...
	plot_scatter(pred=consensus_pred, obs=MDFP_test$pIC50, FP="3DFP11", ML=paste0("consensus", "_seed", seed))
	...
	plot_scatter(pred=consensus_pred_2, obs=MDFP_test$pIC50, FP="3DFP11", ML=paste0("consensus_2", "_seed", seed))
...
write.csv(metric_cv_all, file="3DFP11_metric_cv_all.csv")
write.csv(metric_test_all, file="3DFP11_metric_test_all.csv")

write.csv(metric_cv_all_summary, file="3DFP11_metric_cv_all_summary.csv")
write.csv(metric_test_all_summary, file="3DFP11_metric_test_all_summary.csv")

save.image("3DFP11-ML.rData")
