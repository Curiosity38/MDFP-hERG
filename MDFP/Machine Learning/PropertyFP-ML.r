...
MDFP_train <- filter(PropertyFP, Training.Test == "Training") %>% select(-Name, -Training.Test)
MDFP_test <- filter(PropertyFP, Training.Test == "Test") %>% select(-Name, -Training.Test)
...
for (seed in 2^seq(1,10)){
	...
	plot_scatter(pred=plsFit_pred, obs=MDFP_test$pIC50, FP="PropertyFP", ML=paste0("PLS", "_seed", seed))
