...
MDFP_train <- filter(MDFP_PropertyFP, Training.Test == "Training") %>% select(-Name, -Training.Test)
MDFP_test <- filter(MDFP_PropertyFP, Training.Test == "Test") %>% select(-Name, -Training.Test)
...
rfe_selected_predictors <- c(as.character(MDFP_rfe_predictors$x), as.character(PropertyFP_rfe_predictors$x))
...
for (seed in 2^seq(1,10)){
	...
	plot_scatter(pred=svmFit_pred, obs=MDFP_test$pIC50, FP="MDFP_PropertyFP", ML=paste0("SVM", "_seed", seed))
	...
	plot_scatter(pred=gbmFit_pred, obs=MDFP_test$pIC50, FP="MDFP_PropertyFP", ML=paste0("GBM", "_seed", seed))
	...
	plot_scatter(pred=rfFit_pred, obs=MDFP_test$pIC50, FP="MDFP_PropertyFP", paste0("RF", "_seed", seed))
	...
	plot_scatter(pred=plsFit_pred, obs=MDFP_test$pIC50, FP="MDFP_PropertyFP", ML=paste0("PLS", "_seed", seed))
	...
	plot_scatter(pred=consensus_pred, obs=MDFP_test$pIC50, FP="MDFP_PropertyFP", ML=paste0("consensus", "_seed", seed))
...
write.csv(metric_cv_all, file="MDFP_PropertyFP_metric_cv_all.csv")
write.csv(metric_test_all, file="MDFP_PropertyFP_metric_test_all.csv")

write.csv(metric_cv_all_summary, file="MDFP_PropertyFP_metric_cv_all_summary.csv")
write.csv(metric_test_all_summary, file="MDFP_PropertyFP_metric_test_all_summary.csv")

save.image("MDFP_PropertyFP-ML.rData")
