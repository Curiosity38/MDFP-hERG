...
MDFP_BaselineFP <- left_join(select(MDFP, -pIC50, -Training.Test), BaselineFP, by = "Name")
#5折交叉验证

MDFP_train <- filter(MDFP_BaselineFP, Training.Test == "Training") %>% select(-Name, -Training.Test)
MDFP_test <- filter(MDFP_BaselineFP, Training.Test == "Test") %>% select(-Name, -Training.Test)
...
rfe_selected_predictors <- c(as.character(MDFP_rfe_predictors$x), as.character(BaselineFP_rfe_predictors$x))
...
for (seed in 2^seq(1,10)){
	...
	plot_scatter(pred=svmFit_pred, obs=MDFP_test$pIC50, FP="MDFP_BaselineFP", ML=paste0("SVM", "_seed", seed))
	...
	plot_scatter(pred=gbmFit_pred, obs=MDFP_test$pIC50, FP="MDFP_BaselineFP", ML=paste0("GBM", "_seed", seed))
	...
	plot_scatter(pred=rfFit_pred, obs=MDFP_test$pIC50, FP="MDFP_BaselineFP", paste0("RF", "_seed", seed))
	...
	plot_scatter(pred=plsFit_pred, obs=MDFP_test$pIC50, FP="MDFP_BaselineFP", ML=paste0("PLS", "_seed", seed))
	...
	plot_scatter(pred=consensus_pred, obs=MDFP_test$pIC50, FP="MDFP_BaselineFP", ML=paste0("consensus", "_seed", seed))
...
write.csv(metric_cv_all, file="MDFP_BaselineFP_metric_cv_all.csv")
write.csv(metric_test_all, file="MDFP_BaselineFP_metric_test_all.csv")

write.csv(metric_cv_all_summary, file="MDFP_BaselineFP_metric_cv_all_summary.csv")
write.csv(metric_test_all_summary, file="MDFP_BaselineFP_metric_test_all_summary.csv")

save.image("MDFP_BaselineFP-ML.rData")
