...
# ECFP4 <- mutate(ECFP4, across(!c("Name", "pIC50", "Training.Test"), ~ ifelse(.x >= 1, 1, 0)))
# ECFP4 <- mutate(ECFP4, across(!c("Name", "pIC50", "Training.Test"), as.factor))

MDFP_train <- filter(ECFP4, Training.Test == "Training") %>% select(-Name, -Training.Test)
MDFP_test <- filter(ECFP4, Training.Test == "Test") %>% select(-Name, -Training.Test)
...
for (seed in 2^seq(1,10)){
	...
	plot_scatter(pred=plsFit_pred, obs=MDFP_test$pIC50, FP="ECFP4", ML=paste0("PLS", "_seed", seed))
